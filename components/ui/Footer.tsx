import Image from "next/image";
import Link from "next/link";
import SitioHoyBranding from "./SitioHoyBranding";

const NAV = [
  { href: "/", label: "Inicio" },
  { href: "/catalogo", label: "Propiedades" },
  { href: "/nosotros", label: "Nosotros" },
  { href: "/contacto", label: "Contacto" },
];

export default function Footer() {
  const year = new Date().getFullYear();

  return (
    <footer className="bg-brand-dark text-white">
      {/* CTA Strip */}
      <div className="border-b border-white/[0.06] py-12 px-4 sm:px-6 lg:py-20">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center md:items-center justify-between gap-8 text-center md:text-left">
          <div>
            <h2
              className="font-display font-bold text-white leading-[1.1]"
              style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2rem, 4vw, 3rem)" }}
            >
              ¿Listo para
              <br />
              <em className="text-brand-accent">dar el paso?</em>
            </h2>
            <p className="text-white/45 mt-3 text-sm max-w-sm leading-relaxed">
              Nuestro equipo está disponible para asesorarte de forma gratuita y sin compromisos.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 flex-shrink-0 w-full md:w-auto">
            <a
              href="https://wa.me/5493329696105"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 bg-brand-accent hover:bg-brand-accent/90 text-white font-semibold px-7 py-3.5 text-sm tracking-wide transition-colors"
              style={{ borderRadius: "3px" }}
            >
              <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current flex-shrink-0">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              Escribinos por WhatsApp
            </a>
            <Link
              href="/contacto"
              className="inline-flex items-center justify-center px-7 py-3.5 text-sm font-medium border border-white/15 hover:border-white/30 text-white/70 hover:text-white transition-colors"
              style={{ borderRadius: "3px" }}
            >
              Contacto
            </Link>
          </div>
        </div>
      </div>

      {/* Main footer */}
      <div className="py-10 px-4 sm:px-6 lg:py-16">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr] gap-12">
          {/* Brand */}
          <div>
            <div className="mb-5">
              <Image
                src="/logo.png"
                alt="Sosa Propiedades"
                width={120}
                height={69}
                className="object-contain"
              />
            </div>
            <p className="text-white/35 text-sm leading-relaxed max-w-xs">
              Inmobiliaria arraigada en Baradero desde 2021. Eficiencia, honestidad y compromiso en cada operación.
            </p>
          </div>

          {/* Nav */}
          <div>
            <p className="text-white/25 text-[10px] font-semibold tracking-[0.2em] uppercase mb-5">Navegación</p>
            <ul className="flex flex-col gap-3">
              {NAV.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-white/50 hover:text-brand-accent text-sm transition-colors link-underline"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <p className="text-white/25 text-[10px] font-semibold tracking-[0.2em] uppercase mb-5">Contacto</p>
            <ul className="flex flex-col gap-3 text-sm">
              <li>
                <a href="https://wa.me/5493329696105" target="_blank" rel="noopener noreferrer" className="text-white/50 hover:text-brand-accent transition-colors link-underline">
                  +54 9 3329 69-6105
                </a>
              </li>
              <li>
                <a href="mailto:info@propiedadessosa.com.ar" className="text-white/50 hover:text-brand-accent transition-colors link-underline break-all">
                  info@propiedadessosa.com.ar
                </a>
              </li>
              <li className="text-white/25 text-xs pt-1">
                Lun–Vie · 9 a 18 hs
              </li>
            </ul>
          </div>
        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-white/[0.06] py-5 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] text-white/20">
          <span>© {year} Propiedades Sosa. Todos los derechos reservados.</span>
        </div>
      </div>

      <SitioHoyBranding />
    </footer>
  );
}
